...
@Composable
fun ChangePasswordScreen(viewModel: ChangePasswordVM) {
  val state by viewModel.screenData.collectAsStateWithLifecycle()

  when (val screenData = state) {
    is ChangePasswordVM.ScreenData.Initialized -> ChangePasswordScreenContent(
      data = screenData,
    )
  }
}
...
@Composable
fun ChangePasswordScreenContent(data: ChangePasswordVM.ScreenData.Initialized) {
  BackHandler {
    data.onBackClick()
  }
  ...
  BaseScaffold(
    ...
    bottomBar = {
      Column(
        ...
        LargeButton(buttonData = data.changePasswordButtonData)
      }
    }
  )
}